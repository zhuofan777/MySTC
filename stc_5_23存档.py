...
# 找到较为相近的关键点
def selectKeys(data, alp, blt, ylt, bs):
...
def run(dgo, alp, blt, ylt, clt, bs, lm):
    ...
    # ylt = 5
    keys = selectKeys(data_train, alp, blt, clt, bs)
    ...
    print(sc)
    s = 'D://STC//' + str(alp) + '_' + str(blt) + '_' + str(ylt) + '_' + str(clt) + '_' + str(bs) + '_' + str(
        lm) + '.csv'
    f = open(s, 'a')
...
for i in dgos:
    try:
        run(i, args.alp, args.blt, args.ylt, args.clt, args.bs, args.lm)
    except:
        print(i, " has an error")
        s = 'D://STC//pro.txt'
        f = open(s, 'a')
        f.write(i + str(args.alp) + str(args.blt) + str(args.ylt))
        f.close()
