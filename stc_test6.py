...
# dataset settings
parser.add_argument('--alp', type=float, default=0.7)
parser.add_argument('--blt', type=int, default=30)
parser.add_argument('--ylt', type=int, default=3)
...
def extractFeaturePoint(arr, a, b):
    ...
    stdArrThree = computeStdArr(stdArrTwo, b)
    largePoint = selectLargePoint(stdArrThree, a)
    indexArr = fitPoint(stdArrThree, largePoint, b)
...
def selectKeys(data, alp, blt, ylt):
    ...
    for sm in tqdm(range(sample_nums)):
        ...
        for dn in range(dim_nums):
            ...
            ekl.sort(key = lambda x: x.cnt,reverse=True)
            # print(len(ekl)//5)
            ex_key_points.extend(ekl[:int(len(ekl) * 0.3)])
        # 取前30%的70% 和 后70%的30%
        ex_key_points.sort(key=lambda x: x.cnt, reverse=True)
        # res = []
        # for i in ex_key_points:
        #     if i.cnt >=  dim_nums:
        #         res.append(i)
        #     # if i.cnt < dim_nums:
        #     #     res.append(i)
        ...
        # # 前30%
        # # preLength = int(0.01 * length)
        # res = res[:1]
        # # 后70%
        # aftLength = length - preLength
        # aftKey = ex_key_points[preLength+1:]
        # 取前30%的70%
        ex_key_points = random.sample(ex_key_points,int(len(ex_key_points) * 0.1))
...
def run(dgo, alp, blt, ylt):
    ...
    # 选取shapelet的数量
    k = int(series_length)
    keys = selectKeys(data_train, alp, blt, 3)
...
run('UWaveGestureLibrary', args.alp, args.blt, args.ylt)
